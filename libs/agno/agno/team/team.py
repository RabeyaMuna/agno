...
class Team:
    ...
    events_to_skip: list[RunEvent | TeamRunEvent] | None = None
    ...
    def add_tool(self, tool: Toolkit | Callable | Function | dict):
        if not self.tools:
            self.tools = []
        self.tools.append(tool)
        self._rebuild_tools = True

    def set_tools(self, tools: list[Toolkit | Callable | Function | dict]):
        self.tools = tools
        self._rebuild_tools = True
    ...
    def _run_stream(
        ...
        response_format: dict | type[BaseModel] | None = None,
        stream_intermediate_steps: bool = False,
    ) -> Iterator[TeamRunResponseEvent | RunResponseEvent]:
    ...
    async def arun(
        ...
        *,
        ...
        knowledge_filters: dict[str, Any] | None = None,
        **kwargs: Any,
    ) -> TeamRunResponse | AsyncIterator[RunResponseEvent | TeamRunResponseEvent]:
    ...
    async def _arun_stream(
        ...
        response_format: dict | type[BaseModel] | None = None,
        stream_intermediate_steps: bool = False,
    ) -> AsyncIterator[TeamRunResponseEvent | RunResponseEvent]:
        """Run the Team and return the response.
    ...
    def _handle_model_response_chunk(
        ...
        run_response: TeamRunResponse,
        full_model_response: ModelResponse,
        model_response_event: ModelResponse | TeamRunResponseEvent | RunResponseEvent,
        reasoning_state: dict[str, Any] | None = None,
        stream_intermediate_steps: bool = False,
        parse_structured_output: bool = False,
    ) -> Iterator[TeamRunResponseEvent | RunResponseEvent]:
        if isinstance(model_response_event, (tuple(get_args(RunResponseEvent)), tuple(get_args(TeamRunResponseEvent)))):
    ...
    def _convert_response_to_structured_format(self, run_response: TeamRunResponse | RunResponse | ModelResponse):
    ...
    def _handle_event(self, event: RunResponseEvent | TeamRunResponseEvent, run_response: TeamRunResponse):
    ...
    def _calculate_full_team_session_metrics(self, messages: list[Message]) -> SessionMetrics:
        ...
        for member in self.members:
            ...
            if member.memory is not None:
                ...
                elif isinstance(member.memory, Memory) and member.memory.runs is not None:
    ...
    def get_messages_for_parser_model_stream(
        self, run_response: TeamRunResponse, response_format: dict | type[BaseModel] | None
    ) -> list[Message]:
        """Get the messages for the parser model."""
        from agno.utils.prompts import get_json_output_prompt
    ...
    def get_update_user_memory_function(self, user_id: str | None = None, async_mode: bool = False) -> Function:
    ...
    def get_run_member_agents_function(
        ...
        def run_member_agents(
            task_description: str, expected_output: str | None = None
        ) -> Iterator[RunResponseEvent | TeamRunResponseEvent | str]:
            """
    ...
    def get_transfer_task_function(
        ...
        async def atransfer_task_to_member(
            member_id: str, task_description: str, expected_output: str | None = None
        ) -> AsyncIterator[RunResponseEvent | TeamRunResponseEvent | str]:
            """Use this function to transfer a task to the selected team member.
            You must provide a clear and concise description of the task the member should achieve AND the expected output.
    ...
    def _formate_member_agent_task(
        ...
        expected_output: str | None = None,
        team_context_str: str | None = None,
        team_member_interactions_str: str | None = None,
    ) -> str:
    ...
    def get_forward_task_function(
        ...
        def forward_task_to_member(
            member_id: str, expected_output: str | None = None
        ) -> Iterator[RunResponseEvent | TeamRunResponseEvent | str]:
