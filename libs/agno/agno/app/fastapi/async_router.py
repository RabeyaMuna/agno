...
def get_async_router(
    ...
    async def run_agent_or_team_or_workflow(
        ...
        if workflow_id and workflows:
            ...
            try:
                workflow_input_data: dict = json.loads(workflow_input)
        ...
        if stream:
            if agent:
                ...
                return StreamingResponse(
                    (
                        ...
                        for result in await workflow_instance.arun(**(workflow_input_data or {}))
                    ),
                    media_type="text/event-stream",
                )
        else:
            if agent:
                ...
                workflow_instance.user_id = user_id
                workflow_instance.session_name = None
                return (await workflow_instance.arun(**(workflow_input_data or {}))).to_dict()
