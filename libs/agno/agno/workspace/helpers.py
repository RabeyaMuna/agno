...
def get_workspace_dir_path(ws_root_path: Path) -> Path:
    ...
    if ws_pyproject_toml.exists() and ws_pyproject_toml.is_file():
        ...
        if agno_conf is not None and agno_conf.get("workspace") is not None:
            agno_conf_workspace_dir_str = agno_conf.get("workspace")
            agno_conf_workspace_dir_path = ws_root_path.joinpath(agno_conf_workspace_dir_str)  # type: ignore[arg-type]
